...
WITH stations AS (
    ...
    FROM {{ ref('int_biketrip_hourly_net_flow_dedup') }}
),

date_hours AS (
    SELECT generate_series(
        (SELECT MIN(date_hour) FROM {{ ref('int_biketrip_hourly_net_flow_dedup') }}),
        (SELECT MAX(date_hour) FROM {{ ref('int_biketrip_hourly_net_flow_dedup') }}),
        interval '1 hour'
    ) AS date_hour
),
...
bike_grid AS (
    ...
    FROM full_index fi
    LEFT JOIN {{ ref('int_biketrip_hourly_net_flow_dedup') }} nb
...
merged AS (
    ...
    FROM bike_grid bg
    LEFT JOIN {{ ref('int_weather_hourly') }} wh
...
features AS (
    ...
    FROM merged m
    LEFT JOIN {{ source('raw', 'dim_holidays') }} dh
